...
def generate_context(search_results):
    ...
    # Check if search_results is a dictionary before iterating over its items
    if isinstance(search_results, dict):
        for link_url, data in search_results.items():
            Context += "From " + link_url + " => " + data
    else:
        print("Error: search_results is not a dictionary.")
        return None  # Returning None as an indication of failure
